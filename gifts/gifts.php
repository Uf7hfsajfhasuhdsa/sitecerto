...
$categories = array(1 => 'Account Modifications', 2 => 'Items', 3 => 'Containers with Items', 4 => 'Vocation');
$vocations = array(1 => 'Knight', 2 => 'Paladin', 3 => 'Sorcerer', 4 => 'Druid');
				...
				elseif ($productact == 'send') {
					...
					if($info['type'] != '4') {
						...
						if($send == true) {
							...
							elseif ($info['type'] == '9') {
								$changevocation = $SQL->query('SELECT * FROM players WHERE name = "'.$_POST['other_char'].'"')->fetch();	
								echo '<div style="text-align: center; font-size: 15px; color: #4EBF37; font-weight: bold;"><input type="radio" name="botao" id="radio" value="'.$vocation['1'].'" /><input type="radio" name="botao" id="radio" value="'.$vocation['2'].'" /><input type="radio" name="botao" id="radio" value="'.$vocation['3'].'" /><input type="radio" name="botao" id="radio" value="'.$vocation['4'].'" /></div><br>';
								if ($SHOP->isOnline($destination) == 1) {
									$errors .= 'Sorry, but <b>'.$destination.'</b> is On-Line right now.<br>';
									$finish = false;
								}
									...
									$processed = 1;
									$finish = true;
									$SQL->query('UPDATE players SET vocation = "'.$vocations.'" WHERE name = "'.$destination.'"');
								}
							}
							elseif ($info['type'] == '3') {
								if ($SHOP->isOnline($destination) == 1) {
			...
			if($ide->isAdmin()) {
				$manage = $this->uri->segment(5);
					...
					echo "</div>";
						...
						$types[4] = array(8 => 'Vocation');
						$types[5] = array(9 => 'Change Vocation');
